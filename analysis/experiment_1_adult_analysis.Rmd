...
glmer(cbind(n_atypical, 2-n_atypical) ~ condition + (1|participant_id) + (1 | category), family = "binomial",
      data = model_data, contrasts = list(condition = contr.sum)) %>%
  tidy()
```
...
```{r}
glmer(n_distinct ~ condition + (1|participant_id) + (1 | category), family = "binomial",
      data = model_data, contrasts = list(condition = contr.sum)) %>%
  tidy()
```
