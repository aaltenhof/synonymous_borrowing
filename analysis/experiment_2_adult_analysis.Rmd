...
```{r}
glmer(typicality ~ condition + stimulus_type + (1|participant_id) + (1 | category), family = "binomial",
      data = data, contrasts = list(condition = contr.sum, stimulus_type = contr.sum)) %>%
  tidy()
```
...
```{r}
glmer(distinct_types ~ condition + stimulus_type + (1|participant_id) + (1 | category), family = "binomial",
      data = data, contrasts = list(condition = contr.sum, stimulus_type = contr.sum)) %>%
  tidy()
```
