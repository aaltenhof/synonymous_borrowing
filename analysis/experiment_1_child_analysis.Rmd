...
n_kids_before <- data %>% count(participant_id) %>% nrow()

n_three_before <- data %>% filter(participant_age == 3) %>% count(participant_id) %>% nrow()
n_four_before <- data %>% filter(participant_age == 4) %>% count(participant_id) %>% nrow()
n_five_before <- data %>% filter(participant_age == 5) %>% count(participant_id) %>% nrow()
...
Before exlcusions, we have `r n_kids_before` child participants in the study: `r n_three_before` three-year-olds, `r n_four_before` four-year-olds, and `r n_five_before` five-year-olds.

We exclude `r n_practice_exclusions` participants due to not getting 100% on practice trials. No children are excluded for not finishing the task. After exclusions, we include `r n_kids` child participants in the study: `r n_three` three-year-olds, `r n_four` four-year-olds, and `r n_five` five-year-olds.
...
adult_data <- read_csv(here("../data/e1_adult_data.csv")) %>%
  mutate(age = "Adults") %>%
...
plot_data <- data %>%
  ...
  select(participant_id, trial_number, condition, category, image_name, word, click_order, rt, id, typicality,
         distinct_types, stimulus_subclass, age) %>%
  rbind(adult_data) 
...
ggsave(
  plot = p,
  filename = here("plots/e1_e2_overall_types_effect.svg"),
  height = 5,
  width = 6
)
...
ggsave(
  plot = p,
  filename = here("plots/e1_e2_overall_typicality_effect.svg"),
  height = 5,
  width = 6
)
