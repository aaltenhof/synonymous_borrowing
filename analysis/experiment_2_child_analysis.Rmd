...
data <- raw_data %>%
  ...
  mutate(date_of_birth = if_else(participant_id == 7, "5/27/2021", date_of_birth)) %>%
...
After exclusions, we currently have `r n_kids` child participants in the study: `r n_three` three-year-olds, `r n_four` four-year-olds, and `r n_five` five-year-olds.
...
ggsave(
  plot = p,
  filename = here("plots/e4_age_multiple_types_effect.png"),
  height = 5,
  width = 8
)
...
p <- data %>%
  ...
  geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper)) +
  ylab("proportion atypical choices") +
  facet_wrap(~participant_age) + theme(
  panel.background = element_rect(fill = "transparent",
                                  colour = NA_character_), # necessary to avoid drawing panel outline
  panel.grid.major = element_blank(), # get rid of major grid
  panel.grid.minor = element_blank(), # get rid of minor grid
...
ggsave(
  plot = p,
  filename = here("plots/e4_age_typicality_effect.png"),
  height = 5,
  width = 8
)
...
model_data <- data %>%
  ...
  mutate(participant_id = as.factor(participant_id),
         ...
         stimulus_type = as.factor(stimulus_type))

model_data %>%
  group_by(condition) %>%
  summarise(mean(n_atypical/2))

model_data %>%
  group_by(condition) %>%
  summarise(mean(n_distinct))

contrasts(model_data$condition) = contr.sum(2)
contrasts(model_data$stimulus_type) = contr.sum(2)
...
glmer(cbind(n_atypical, 2-n_atypical) ~ condition * differential + exact_age + stimulus_type + (1 | participant_id) + (1 | category), family = "binomial",
      data = typicality_model_data, contrasts = list(condition = contr.sum, stimulus_type = contr.sum)) %>%
  tidy()

glmer(n_distinct ~ condition * differential + exact_age   + stimulus_type + (1 | participant_id) + (1 | category), family = "binomial",
      data = typicality_model_data, contrasts = list(condition = contr.sum, stimulus_type = contr.sum)) %>%
  tidy()
```
...
```
```{r age-plots}
# examine effect of age we find in atypical choices
data %>%
  filter(trial_number > 1) %>%
  group_by(condition, participant_age, participant_id) %>%
  summarise(prop_atypical = sum(typicality)/n()) %>%
  ...
  tidyboot_mean(prop_atypical) %>%
  ungroup() %>%
  ggplot(aes(x = participant_age, y = mean, color = condition)) + 
  geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper)) +
  ylab("proportion atypical choices") 

# examine effect of stimuli we find in atypical choices
data %>%
  filter(trial_number > 1) %>%
  group_by(condition, stimulus_type, participant_id) %>%
  summarise(prop_atypical = sum(typicality)/n()) %>%
  ungroup() %>%
  group_by(condition, stimulus_type) %>%
  tidyboot_mean(prop_atypical) %>%
  ungroup() %>%
  ggplot(aes(x = stimulus_type, y = mean, color = condition)) + 
  geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper)) +
  ylab("proportion atypical choices") 
```
