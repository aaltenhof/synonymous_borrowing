...
data <- raw_data %>%
  ...
  mutate(participant_id = if_else(participant_id == 15 & participant_age == 4, 14, participant_id), # manually fix errors in age + id logging
         ...
         participant_id = if_else(participant_id == 48 & as.character(session_time) == "14:21:16", 55, participant_id)) %>%
  filter(participant_id != 43) # exclude second run of participant who was run twice
# note: we've moved a second participant 48 to subid 55, but this participant currently doesn't have a row in our separate participant sheet
